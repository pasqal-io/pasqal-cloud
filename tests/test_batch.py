...
class TestBatch:
    ...
    def test_create_batch_only_no_retries_on_transient_errors(
        self,
        mock_request,
        exception,
        expected_exception,
    ):
        """Test that POST create batch call do not retry
        on transient errors"""
        mock_request.reset_mock()
        ...
        # There is only one call to create the batch
        assert mock_request.call_count == 1
        assert mock_request.last_request.method == "POST"
        assert mock_request.last_request.path == "/core-fast/api/v1/batches"
