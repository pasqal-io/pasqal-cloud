...
To authenticate with [`PasqalCloudConnection`][pasqal_cloud.pasqal_cloud_connection.PasqalCloudConnection], you need to enter the credentials
of your account as well as a project ID you are a member of. The jobs created through the connection will then be billed
to this project.
...
Authenticate with your email and password directly:
...
The password can be omitted from the arguments. You will be prompted to input your password directly in your terminal
...
connection = PasqalCloudConnection(token_provider=CustomTokenProvider(), project_id=project_id)
```
...
For OVH users, authentication is handled via the `PASQAL_PULSER_ACCESS_TOKEN` environment variable.
No username or password is required.

```bash
export PASQAL_PULSER_ACCESS_TOKEN="your-access-token"
```
